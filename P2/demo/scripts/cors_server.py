...
class CustomRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_OPTIONS(self):
        self.send_response(200)
        self.send_header('Access-Control-Allow-Origin', '*') # Allow external origins
        self.send_header('Access-Control-Allow-Methods', '*') # Allow all HTTP methods (GET,POST,PUT,DELETE,..) from external origins
        self.end_headers()
